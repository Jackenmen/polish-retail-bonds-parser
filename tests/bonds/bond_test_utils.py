            ...
            for period, period_line in zip(range(period_count), it):
                ...
                if not period_line:
                    period_start = add_months(
                        series_sale_from, period * months_per_period
                    )
                    period_end = add_months(
                        series_sale_from, (period + 1) * months_per_period
                    )
...
def assert_common_bond_traits(bond: Bond) -> None:
    ...
    assert all(
        ...
        for value in actual_total_redemption_values[
            first_period.start : first_period.end
        ]
    )
